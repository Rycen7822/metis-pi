...
export async function injectPendingNativeWindowIntoPiCompactionRequest(payload, ctx, state) {
    const pending = state.pendingPiCompactionNativeWindow;
    if (!pending || pending.window.length === 0)
        return undefined;
