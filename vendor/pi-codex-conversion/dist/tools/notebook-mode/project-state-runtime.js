...
export function parseProjectBindingNames(result, marker) {
    if (result.status !== "ok")
        throw new Error(result.errorText ?? "Project binding selection failed");
    const output = result.items.filter(({ type }) => type === "input_text").map(({ text }) => text ?? "").join("");
    const start = output.indexOf(marker);
    if (start === -1)
        throw new Error("Project binding selection returned no result");
    try {
...
export function promoteProjectBindingsSource(names) {
...
export function syncProjectBindingsSource(names) {
...
export function projectStateRestoreSource(manifest, payloadPath, clearNames = []) {
