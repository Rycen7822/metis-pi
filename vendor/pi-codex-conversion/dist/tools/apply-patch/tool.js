...
function expectedContextPreview(cause) {
...
function summarizePatchCause(cause) {
...
function addContextRecovery(message, cause, failedTargets) {
    if (expectedContextPreview(cause) === undefined)
        return message;
...
export { clearApplyPatchRenderState, isApplyPatchToolDetails } from "./render-state.js";
...
export function createApplyPatchTool(options = {}) {
    ...
    const defaultRenderResult = (result, { isPartial }, theme, context) => {
        if (compactRendering(context)) {
            if (isPartial)
                return renderCompactApplyPatchCall(theme);
            if (isApplyPatchToolDetails(result.details)) {
                if (result.details.status === "partial_failure")
                    return new Text(theme.fg("error", "• Patch partially failed"), 0, 0);
                return new Text(theme.fg("dim", `• Applied patch (${summarizePatchCounts(result.details.result)})`), 0, 0);
            }
            if (context.isError)
                return new Text(theme.fg("error", "• Patch failed"), 0, 0);
    ...
    return {
        ...
        async execute(toolCallId, params, signal, _onUpdate, ctx) {
            ...
            const details = { status: "success", result };
            recordApplyPatchDisplayOutcome(toolCallId, {
                content: summary,
                details,
                isError: false,
