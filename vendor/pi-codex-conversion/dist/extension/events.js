import { readEffectiveCodexConversionConfig } from "../adapter/activation/config-store.js";
...
import { hasPortableNativeCompactionSummary, isNativeCompactionDetails, NATIVE_COMPACTION_DISPLAY_MESSAGE_TYPE, NATIVE_COMPACTION_DISPLAY_TEXT, NATIVE_COMPACTION_PORTABLE_DISPLAY_TEXT, NATIVE_COMPACTION_STRATEGY } from "../adapter/compaction/types.js";
...
import { rewriteCodexProviderHeaders, rewriteCodexProviderRequest, supportsCodexDeveloperMessages } from "../adapter/provider-request.js";
...
import { prepareVoiceDelegation } from "../voice/delegation-preflight.js";
import { appendNotebookTreeEpoch } from "../tools/notebook-mode/session-identity.js";
import { formatCompactionCacheDiagnostic } from "../adapter/compaction/diagnostics.js";
import { registerCodexDeveloperMessageBroker, updateCodexPreparedIdleKickoff } from "../developer-messages.js";
import { isContextWindowCompactionDetails } from "../context-management/messages.js";
import { flushCodexReasoningUpdates, recordCodexReasoningUpdate } from "../adapter/reasoning-updates.js";
import { createCodexReserveController } from "../codex-usage/reserve.js";
...
export function registerCodexEvents(pi, runtime, tools, ui, codeMode, proxyProvider) {
    ...
    let turnPrewarm;
    const unregisterDeveloperMessageBroker = registerCodexDeveloperMessageBroker(pi, () => Boolean(activeContext &&
        supportsCodexDeveloperMessages(activeContext, state)));
    const unregisterExtensionToolRefresh = onCodeModeExtensionToolsRefresh(pi, () => {
        if (!activeContext)
            return;
    ...
    pi.events.on(REALTIME_VOICE_PROMPT_CHANNEL, (value) => {
        ...
        if (report)
            runtime.voice.setPrompt(report);
    });
    ...
    pi.on("thinking_level_select", (event, ctx) => {
        if (supportsCodexDeveloperMessages(ctx, state))
            recordCodexReasoningUpdate(pi, ctx, runtime.projectContextMessages(ctx), event.previousLevel);
    });
    ...
    pi.on("session_before_tree", (event, ctx) => {
        if (state.contextTree.handoff.active)
            return { cancel: true };
        if (state.contextTree.archiving)
            return;
        const plan = resolveCodexRuntimePlanForState(ctx, state);
        if (!plan.contextManagement || !event.preparation.userWantsSummary)
            return;
    ...
    pi.on("session_tree", async (event, ctx) => {
        ...
        state.voiceSystemPromptOverride = undefined;
        runtime.resetTransport(ctx.sessionManager.getSessionId());
        if (state.contextTree.handleSessionTree(event))
            return;
        if (previousMode === "notebook" || state.executionMode === "notebook")
            appendNotebookTreeEpoch(pi);
    ...
    pi.on("message_end", async (event) => {
        if (event.message.role === "assistant") {
            runtime.voice.finishAgentMessage(event.message, state.config.voice.forwardReasoningSummaries);
    ...
    pi.on("turn_end", (event, ctx) => {
        ...
        if (state.contextTree.handoff.active)
            return;
        state.contextWindows.recordBudget(pi, ctx, plan.contextManagement);
    });
    ...
    pi.on("before_agent_start", async (event, ctx) => {
        ...
        turnPrewarm = runtime.waitForPrewarm(ctx, codexSystemPrompt)?.catch((error) => {
    ...
    pi.on("agent_settled", async (_event, ctx) => {
        ...
        try {
            ...
            if (continuingWork && !continued && !state.contextWindows.isHybridCompactionRunning())
                runtime.autoReasoning.settle(ctx);
        }
        if (!rolled && !continued && !quotaExhausted && !state.contextWindows.isHybridCompactionRunning())
    ...
    pi.on("session_before_compact", async (event, ctx) => {
        ...
        const contextManagementResult = plan.contextManagement
            ? state.contextWindows.prepareCompaction(event, plan.contextManagementMode, plan.contextManagementHybrid)
            : undefined;
    ...
    pi.on("session_compact_failed", async (event, ctx) => {
        if (state.contextWindows.isHybridCompactionRunning())
            runtime.autoReasoning.settle(ctx);
        state.pendingPiCompactionNativeWindow = undefined;
        runtime.voice.compactionFinished();
        const plan = resolveCodexRuntimePlanForState(ctx, state);
        const reuseNotes = state.contextWindows.finishManualCheckpointRequest(pi, ctx, event, plan.contextManagement && !plan.contextManagementHybrid);
        if (!reuseNotes)
            return;
        try {
    ...
    pi.on("session_compact", async (event, ctx) => {
        try {
            ...
            const contextCompaction = event.fromExtension &&
            ...
            if (event.fromExtension && compactionEntry && isNativeCompactionDetails(compactionEntry.details)) {
                ...
                pi.appendEntry(NATIVE_COMPACTION_DISPLAY_MESSAGE_TYPE, {
            ...
            if (plan.contextManagementHybrid) {
                if (plan.contextManagementMode === "tree" && compactionEntry) {
                    ...
                    await state.contextWindows.completeHybridCompaction(pi, ctx, plan.contextManagementMode);
            }
    ...
    pi.on("context", async (event, ctx) => {
        ...
        return {
            messages: state.developerMessages.prepare(messages, supportsCodexDeveloperMessages(ctx, state), ctx.model),
        };
    });
}
