...
function resolveProviderApis(config, executionMode, modelRegistry) {
    const resolved = new Map();
    for (const model of modelRegistry.getAll()) {
        if (model.api !== "openai-responses" &&
            model.api !== "openai-codex-responses")
            continue;
        const api = model.api === "openai-responses"
            ? "openai-responses"
...
export function registerCodeModeProxyProvider(pi, getConfig, getExecutionMode = () => undefined, getAvailableToolNames = () => undefined) {
    ...
    const restoreProvider = (provider, registration) => {
        ...
        else if (!registration.previous?.streamSimple && current.api === registration.api)
            delete restored.api;
    ...
    const applyConfig = (config, modelRegistry) => {
        const desiredProviders = resolveProviderApis(config, getExecutionMode(), modelRegistry);
        for (const [provider, api] of desiredProviders) {
            ...
            const overlayStream = (model, context, options) => {
                const plan = resolveCodexRuntimePlanForState({ model }, {
                    config: getConfig(),
                ...
                const stream = fallbackProvider.streamSimple(model, context, options);
