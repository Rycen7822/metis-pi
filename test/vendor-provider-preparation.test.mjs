...
function fixture(options = {}) {
	...
	const state = {
		...
		developerMessages: {
			rewritePayload(payload, currentModel) {
				order.push("developer");
				assert.equal(payload.instructions, "VOICE_INSTRUCTIONS", "voice options precede developer rewriting");
				assert.equal(payload.text.verbosity, "high", "request options precede developer rewriting");
				return bridge.rewritePayload(payload, currentModel);
			},
		},
		contextWindows: {
			rewritePayload(payload, context) {
				order.push("window");
				assert.equal(payload.input[0].role, "developer", "developer rewriting precedes window metadata");
				assert.equal(payload.tools[0].type, "namespace", "namespaces precede window metadata");
				assert.equal(payload.tools[1].tools.find((tool) => tool.name === "write_file").parameters.properties.text.encrypted, true);
				return rewriteWindowPayload(payload, context, { firstWindowId: "window-0", currentWindowId: "window-1", windowNumber: 1 });
			},
		},
	};
	...
	return { ctx, state, payload, order };
}

for (const scenario of cases) {
	test(`${scenario.label}: live and prewarm share ordered preparation without sharing final side effects`, async () => {
		const { ctx, state, payload, order } = fixture(scenario);
		...
		assert.deepEqual(order, scenario.remote ? ["developer", "window", "developer", "window"] : ["developer", "developer"]);
...
test("inactive and unsupported requests do no preparation or final work", async () => {
	for (const reason of ["voice-only", "missing-tools", "unconfigured", "unsupported-api", "extras"]) {
		const { ctx, state, payload, order } = fixture();
...
function pendingFixture() {
	...
	const sm = SessionManager.inMemory("/tmp/offline-provider-preparation");
	const kept = sm.appendMessage({ role: "user", content: "kept", timestamp: 1 });
	const sourceCompactionEntryId = sm.appendCompaction("[OpenAI native compaction checkpoint]", kept, 100, {
		strategy: "openai-responses-compaction-v2", provider: model.provider, api: model.api, model: model.id,
		baseUrl: model.baseUrl, createdAt: new Date(2).toISOString(), compactedWindow: [SEALED_WINDOW_ITEM],
	});
	let authCalls = 0;
	const ctx = {
		model, sessionManager: sm,
		modelRegistry: { getApiKeyAndHeaders: async () => { authCalls++; return { ok: true, apiKey: FAKE_API_KEY, headers: {} }; } },
	};
	const pending = {
		window: [structuredClone(SEALED_WINDOW_ITEM)], provider: model.provider, api: model.api,
		baseUrl: model.baseUrl.replace(/\/+$/, ""), sessionId: sm.getSessionId(), sourceCompactionEntryId,
	};
	const state = {
		config: structuredClone(DEFAULT_CODEX_CONVERSION_CONFIG),
		developerMessages: new CodexDeveloperMessageBridge(),
		pendingPiCompactionNativeWindow: pending, pendingActiveProviderPromptCapture: true, activeProviderSystemPrompt: "BEFORE_CAPTURE",
	};
	const payload = { model: model.id, instructions: "Summarize this conversation", input: [{ role: "user", content: "VISIBLE_TAIL" }] };
	return { ctx, state, pending, payload, authCalls: () => authCalls };
}

test("ordinary prewarm does not consume a pending window; only the live/final summary injects and captures it", async () => {
	const { ctx, state, pending, payload, authCalls } = pendingFixture();
	const prewarm = rewriteCodexPrewarmProviderRequest(payload, ctx, state);
	...
	assert.equal(JSON.stringify(prewarm).includes("offline-sealed-fixture"), false);
	const live = await rewriteCodexProviderRequest(payload, ctx, state);
...
test("ordinary prewarm never triggers native replay, even when native compaction is enabled", () => {
	const { ctx, state, pending, payload, authCalls } = pendingFixture();
	state.config.compaction.responsesCompaction = true;
	assert.equal(resolveCodexRuntimePlanForState(ctx, state).nativeCompaction, true);
	const prewarm = rewriteCodexPrewarmProviderRequest(payload, ctx, state);
...
test("live non-summary preserves a pending window, while a rejected summary clears it", async () => {
	const { ctx, state, pending, payload, authCalls } = pendingFixture();
	...
	const result = await rewriteCodexProviderRequest(payload, ctx, state);
	assert.equal(state.pendingPiCompactionNativeWindow, undefined);
...
test("failed common preparation stops before prompt capture and pending-window consumption", async () => {
	for (const boundary of ["developerMessages", "contextWindows"]) {
		for (const rewrite of [rewriteCodexPrewarmProviderRequest, rewriteCodexProviderRequest]) {
			const { ctx, state, payload, order } = fixture({ mode: "remote" });
			...
			assert.equal(state.activeProviderSystemPrompt, "BEFORE_CAPTURE");
			assert.equal(state.pendingPiCompactionNativeWindow, pending);
			assert.deepEqual(order, boundary === "developerMessages" ? [] : ["developer"]);
		}
	}
});
