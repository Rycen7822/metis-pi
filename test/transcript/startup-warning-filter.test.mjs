...
test("only the exact known fallback is hidden; unknown warnings and extra diagnostics survive", () => {
  const calls = [];
...
test("replacement ownership, late teardown and foreign wrappers remain safe", () => {
  const calls = [];
  const target = { warn: (...args) => calls.push(args) };
  const original = target.warn;
  ...
  target.warn(warning);
  assert.deepEqual(calls, [[warning]], "a disposed filter in a foreign chain is inert");
  target.warn = original;
});

test("TUI lifecycle filters child startup output; headless/RPC children cannot remove the parent filter", async () => {
  const calls = [];
