...
test("prewarms one STA worker and preserves exact UTF-8/base64, including empty text", async (t) => {
  ...
  output("K".repeat(texts.length));
  assert.deepEqual(await Promise.all(copies), texts.map(() => true));
  assert.equal(spawn.mock.callCount(), 1);
  assert.equal(child.kill.mock.callCount(), 0, "successful copies keep the worker alive");
});

for (const warm of [false, true]) test(`${warm ? "warm" : "cold"}: readiness gates FIFO writes and split/coalesced acknowledgements`, async (t) => {
  const { clipboard, output, writes } = fixture(t);
  if (warm) output("R");
  const completed = [];
  const copies = ["first", "second", "third"].map((text, index) => clipboard.copy(text).then((result) => {
  ...
  if (!warm) {
    ...
    assert.deepEqual(await values([clipboard.ready]), ["pending"]);
    output("R");
  }
  assert.equal(await clipboard.ready, true);
  await flush();
  assert.deepEqual(writes(), ["Zmlyc3Q=\n", "c2Vjb25k\n", "dGhpcmQ=\n"]);
  output("K");
  await flush();
  assert.deepEqual(completed, [0]);
  output("KK");
  assert.deepEqual(await Promise.all(copies), [true, true, true]);
