...
| 命令 | 范围 |
| --- | --- |
| `npm run verify` | 全部测试、项目/vendor 类型检查、vendor 激活、真实 Pi 宿主 smoke、包内容 dry-run。 |
| `npm run check:core` | `src/` 类型检查。 |
| `npm run test:host` | 真实 Pi 组件的工具槽、显示和宿主交互契约。 |
| `npm run test:chrome` | chrome 模块边界和数据形状。 |
...
| `node scripts/copy-perf.mjs` | 复制路径的性能测量。 |

先跑改动附近的测试；跨模块改动完成后运行 `verify`。渲染/交互变动还应运行 PTY；缺 Pi/tmux 时该脚本会 SKIP，退出 0 也不代表交互已验证。当前滚轮环境限制见 [VALIDATION](../VALIDATION.md)，不得删除正式断言来取得通过。`NO_COLOR` 会影响颜色相关检查，必要时使用 `env -u NO_COLOR`。
