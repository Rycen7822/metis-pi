...
用户看到面板上 `#12 #13 #14` 后提问："为什么接着之前完成的 11 个继续编号？"——因为 0.19.4 为了"陈旧引用
必须响亮失败"保留了单调递增的内部序号，并把同一串数字直接当成对外编号显示。现在把两件事分开：

- **对外编号 = 层级路径**：顶层任务按顺序 `#1`、`#2`…，子任务跟在父路径后面点数（`#1.1`、`#1.2`、`#1.1.1`，
  最多 4 层）。面板、`todo list` 文本、工具结果、工具入参全都用这一套；`add` 的 `parentId`、
  `complete`/`skip`/`update` 的 `id`、`addBlockedBy` 的 `blockedBy` 都接受 `"1.2"` 形式（`#` 可省，
  顶层也接受数字 1，同一批次内先建父再挂子仍然一次调用搞定）。
- **每张新列表从 `#1` 重新开始**（取代 0.19.4 的"继续递增"），所以面板读起来是一份新计划而不是账本尾巴。
- **内部数字 id 退居存储层**，不再出现在任何输出里（`formatTaskId` 只被模型层的错误文本使用，工具层统一
  用 `translateIds` 把 `#13` 这类内部编号改写为路径后再返回）。
- **找不到就教学**：解析失败会列出当前所有路径（`task #2 not found — current paths: #1, #1.1`），
  过期或结构变化后的引用能自我纠正，而不是静默打到别的任务上。
- 代价与对策：路径只在**同一张列表内**唯一，所以新列表开场时 `add` 的回报里写明
  `(new list: M finished task(s) cleared; ids restart at #1 — earlier #id references are void)`；
  这是唯一会复用编号的时刻，工具描述也写明了"不要复用重启前读到的 id"。
  轮换时旧行不会残留：面板直接由 store 渲染，旧任务已删除，因此不会出现新旧同号并存。

验证：`test/todo-model.test.mts` 新增 `taskPaths` 层级编号、`resolveTaskRef`（含错误信息）、
"提升子任务后位置重排"三例；`todo-tools`/`todo-widget`/`todo-extension` 的断言改为路径形式
（`#1.1 write store`、`◐ #1.1 child · mine`、轮换后的 `#1` 与陈旧引用 `task #2 not found — current paths: #1`），
全套 403→**406/406** 通过；`check` 0；`test:host` PASS；pty rc=0。
...
用户反馈：截图里有 14 行删减，footer 无论等多久都只显示 6 行。复现与根因（真实 git 驱动真实 tracker）：

1. **数学无效的相减**：旧实现用同一个 HEAD 为锚取两次 `git diff --numstat` 再相减，当编辑与"会话开始前就
   存在的未提交改动"重叠时该减法不成立，`max(0, …)` 直接把整段吞掉 —— 实测在既有 WIP 内部重写 3 行，
   真实 `+3 -3` 被报成 `+0 -0`；随后追加 4 行，真实 `+7 -3` 被报成 `+4 -0`。
2. **口径本身看不见"加了又删"**：任何"当前状态 vs 基线"的对比都无法看见被自己撤销的改动。截图里的探针代码
   从未进入 git 历史（`git log -S` 已确认），因此那 14 行删除在任何状态口径下都不可能出现。

新口径：每次读取把每个变更路径的**内容**与"本 session 上次看到的内容"做真实 diff 并累加 —— 加过的行再删掉
同样计入删除（实测同一个场景 `+19 -14`，与编辑面板逐条相加一致）。三条既有规则保留：session 第一次读取是
内容基线（会话前的未提交改动永不计入）；commit 把已提交增量折掉；工作区干净即归零。

实现：`git hash-object -w --no-filters` 把变更内容写进 **session 私有临时对象库**（`GIT_OBJECT_DIRECTORY`
指向 OS 临时目录，用户的 `.git/objects`、index、工作区一个字节都不写；`--no-filters` 绕开 LFS/smudge/diff
驱动），再用 `git diff --numstat <旧blob> <新blob>` 取精确增删。无法引用内容的路径退回"计数增长"降级路径，
失败绝不重复计数也绝不清零。内容引用按 path 保存（≤2000 条，超出 FIFO），仅对**本次读取中变更过**的路径做
一次 hash 与一次 blob diff。

验证：400/400（`test/git-changes.test.mts` 17 例，含"加了又删仍计入删除""会话前 WIP 内部编辑精确计数"
"未跟踪文件增长与缩短"三条真实 git 回归）；check 0；test:host PASS；pty rc=0（新增端到端断言：改写本会话
刚加的 3 行后 footer 读 `+5 -3`，而工作区 vs HEAD 只有 `+2 -0`）。`/codex-ui` 现在同时输出 churn、读取次数
与工作区 vs HEAD 的原始值，便于逐项对账。
...
根因在宿主：用户消息的 skill 解析器 `parseSkillBlock` 用的是**锚定的单块正则**
（`^<skill …>([\s\S]*?)</skill>(?:\n\n([\s\S]+))?$`），只认行首**一个** skill 块；
第一个 `</skill>` 之后的所有内容都会被当成"用户正文"交给普通文本组件**原样展开渲染**。
我们用兄弟块拼接（`block A\n\nblock B`）时，B 正好落进那个"用户正文"→ 于是 A 折叠、B 展开。

修法：把后续块**嵌套**进前一个块的正文里（`A{ 正文A + B{ 正文B } }`，链式嵌套，标签仍平衡）：

- 模型的输入不变实质：每个 skill 仍是宿主格式的 `<skill name="…">` 块，正文逐字保留、顺序不变；
- 宿主只解析出一个块 → 转写区只显示**一条可折叠的 `[skill] …`**，默认收起，正文不再外泄；
- 真正尾随的用户文字仍然留在块外（`…</skill>\n\n用户文字`），照旧渲染成用户消息。
- 三块以上同样成立（链式嵌套让"第一个 `</skill>`"只可能是最外层那个的收尾）。

验证：387/387；check 0；`test:host`（**用宿主真实的 `parseSkillBlock`** 验证：解析出单个块、name=第一个
skill、正文包含第二个 skill、userMessage 只剩余尾随文字）；pty rc=0——转写区断言出现 `[skill] pcx-pty-mux-a`
且 **skill 正文不可见**（折叠住），模型的 mock 仍收到两块 + 尾随。

## 0.18.2

**去掉 0.18.0 的 `继续添加 skill` 提示行**（用户要求）。菜单弹出已由 0.18.1 的 composer 查询钩子接管，
提示行不再承担触发职责，删掉后：

- 完整 skill token + 空格处**不再多一行**；紧接着的 `/` 由编辑器钩子补发查询 → 菜单照常按下即弹
  （状态活着、状态已死两种路径都验证过）。
- 同时删掉 `BRIDGE_ITEM` / `matchSkillBridge` 与包装器里的哨兵接受分支；`src/skill-tokens.ts`
  （`splitLeadingSkillHeads` / `isSkillPrefixOnly`）保留——它现在是 composer 钩子的共享 token 定义。
- 补全器回归到"只补第二个及以后 token"的单一职责，Tab 强制查询的门控仍在。

验证：387/387；check 0；pty rc=0（裸 `/` 弹出菜单且断言**不存在** `继续添加 skill` 行）。

## 0.18.1

**第二个 `/` 真正按下即弹**——0.18.0 的"续状态"只在"有查询发生"时成立，用户实测仍有一条路径没覆盖：
**菜单状态已经死掉之后敲 `/`**（典型：按 Tab 接受了第一个 token、或 Esc 关掉菜单；接受路径里宿主
硬编码 `cancelAutocomplete()`，之后空格键根本不会发起查询，于是 `/` 也不会）。

- **在编辑器层补一次查询**：宿主的自动触发规则里，`/` 只在**行首**触发（`isAtStartOfMessage`），
  且 `/` 被显式排除在补全触发字符之外；状态死掉后 `/` 按键**零查询**——补全器根本没被调用，
  所以在补全器里怎么改都没用。真正能补的是编辑器本身：本包的 Codex composer 已经是
  `CustomEditor` 子类并覆写 `handleInput`，现在在插入 `/` 之后判断「前面是完整 skill token + 空白」时，
  调用宿主编辑器自己的自动触发（与"斜杠上下文里敲字母"同一条查询路径），菜单立刻弹出。
- 判定用新抽出的零依赖模块 `src/skill-tokens.ts`（`splitLeadingSkillHeads` / `isSkillPrefixOnly`），
  与补全器共享同一份 token 定义，不会漂移；只在**真正的位置**触发（普通文字里、路径里 `src/`、
  非 `/skill:` 开头的命令都不介入）。
- 菜单已经打开时不重复查询；宿主若把该私有方法改名，钩子静默失效而不是崩（可选成员 + 类型守卫）。
- 0.18.0 的"续状态提示行"保留为兜底（chrome composer 不可用时仍能弹），关掉它只是个 UI 取舍。

验证：387/387；check 0；pty rc=0 且覆盖**死状态**路径——Esc 杀掉菜单状态 → 空格 → **裸 `/` 自己弹出菜单**
（无 Tab、无字母）→ 过滤 → 接受 → 提交 → mock 回报双块 + 尾随、无残留 token。

## 0.18.0

**第二个 `/` 按下即弹菜单**——0.17.9 的发现在用户实测后不够用（Tab 是绕路）。真正的钥匙是**别让菜单状态在空格处死掉**：

- 编辑器在补全查询返回空时 `cancelAutocomplete()`（清 `autocompleteState`）；内置补全器对 `/skill:a `（skill 命令无参数补全）
  返回空 → 状态死 → 之后裸 `/` 按键**根本不会发起任何查询**（pi-tui 只在行首自动触发 `/`）。
- 修法：包装器在「**完整且真实存在的 skill token + 纯空白**」位置返回一行**无副作用提示项**
  （`继续添加 skill` / `输入 / 或 ￥`），只为把菜单状态续住；于是紧接着的 `/` 或 `￥` 按键走
  `updateAutocomplete()` 回到本插件，弹出真正的 skill 菜单。
- 提示项安全性：`value` 用哨兵（不会被渲染，SelectList 只渲染 label），`applyCompletion` 对哨兵**原文返回不动**，
  `prefix` 以 `/` 开头 → 编辑器 `cancelAutocomplete()` 后 **fall through 到正常提交**：Enter 照常发送消息，
  不会被菜单吞掉；Tab 只是关掉这行提示（文字不变）。
- 位置判定严格：必须是**已知 skill** 的完整 token、且 token 之后只有空白（`matchSkillBridge`），
  未知名字、半个 token、普通文字、`/todos` 之类非 skill 命令一律不介入；内置补全器能回答的位置仍归内置。
- 无状态的兜底仍在：0.17.9 的 Tab 强制查询门控保留（Esc 关掉菜单后按 Tab 依然出菜单）。

验证：386/386；check 0；pty rc=0——真实 E2E 覆盖用户原始复现路径：`/skill:pcx-pty-mux-a ` → 提示行出现 →
**裸 `/` 自己弹出菜单**（无 Tab、无多余字母）→ 过滤 → 接受 → 提交 → mock 回报双块 + 尾随、无残留 token；
另含 Esc 置死状态 + Tab 兜底回归。
...
- 常驻列表由 2 行变 **3 行**（`maxLines` 4 → 5：标题 + 3 行任务 + `+N more` 汇总 + 间隔行）。
- 折叠语义换成两级视图：默认**三行常驻**，**左键单击面板任意一行**展开成完整列表（汇总行消失，
  标题变 `▴ · click to collapse`），**再点一次收回三行**。展开态持久化（`widgetExpanded`，
  替代原 `widgetFolded`）；`ctrl+shift+t` 保留为键盘等价物，但标题不再提示它。
- 实现：组件的 `handleMouse` 认领左键 click（宿主全屏模式按 layout 命中测试派发鼠标事件，
  组件实例是持久的），非左键/非 click 事件原样放行；点击后重置高度 latch 使面板可缩放，
  实时更新仍只增不减（防终端跳动）。
- 溢出裁剪策略整理：汇总行与它汇总的行共享预算，先丢最早完成的、再截断 pending 尾部，
  存活行保持树序（原实现会把保留的已完成行提到最前面）。

验证：370/370（widget 测试 7 → 9，新增点击切换与"展开/收起可改高、实时更新不缩行"）、check 0 error、
pty rc=0 且新增真实鼠标阶段（5 个任务 → 断言行数为 3 + `+2 more (0 completed, 2 pending)` →
点击后 5 行且无汇总 → 再点回 3 行）。pty 顺带修两处因面板变高而暴露的脆弱断言：
诊断块顶部（speed 段）需要先向上滚动再断言，读完后滚回实时尾部；`/hotkeys` 断言跟着快捷键描述更新。
