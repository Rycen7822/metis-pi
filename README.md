...
- **紧凑产品 footer**：`目录 (分支) +新增 -删除   N tok/s · ↑input ↓output · cache 命中率 · Codex 5h 82% · week 64%`；分支右侧的 `+A -D` 是**本 session 观察到的累计改动行数（churn）**（绝对值，不是净变化；**加过又删掉的同样计入**；commit 折掉已提交部分、工作区干净即归零；用与 diff 相同的绿/红，始终精确到个位，干净仓库不显示）；`N tok/s` 是**实测输出速度**，位于右块最左、紧跟其后才是 `↑input`。窄屏按 优先级 降级（缩短目录 → 两行），P0/P1 永不整块消失；不与 metadata 重复显示 model/context。
...
- **常驻面板**：`Todos 2/5 done` 标题 + 树形行（`○ ◐ ✓ ✗ ⚠︎` 状态符，有 blockedBy 时才显示层级路径 `#1.1`），
...
- **列表生命周期（0.19.4）**：**默认新建**——对一个已完成（全部 complete/skipped）的列表再 `add`，会**开一张新列表**
  （旧任务清空，绝不把历史任务追加到新工作后面）；只有列表**还有未完成任务**时才追加，也就是"非必要不追加"。
  已完成列表本身就是历史（`gcDays` 会回收），store 只保存**正在做的那张列表**。
- **任务编号（0.19.5）**：编号是**层级路径**——顶层任务从 `#1` 起、子任务是父任务的 `#1.1` / `#1.2`，孙辈 `#1.1.1`
  （最多 4 层）；**每张新列表都从 `#1` 重新开始**。工具入参用同一套路径（`"1"`、`"1.2"`，`#` 可省；顶层也接受数字），
  找不到时错误会列出当前所有路径（`task #2 not found — current paths: #1, #1.1`），便于自我纠正。
  代价与对策：路径只在**同一张列表内**唯一，所以新列表开场时 `add` 会明确回报
  `(new list: M finished task(s) cleared; ids restart at #1 — earlier #id references are void)` ——
  这一句就是给模型的"旧引用作废"提示（旧编号从不出现在界面或工具输出里，内部数字 id 仅用于存储）。
- **鼠标点击展开**：面板默认显示**三行任务**，超出时尾部折叠成 `+N more (a completed, b pending)`
  ...
  **右键（按下即触发，无需松开——Warp 等终端会吃掉右键释放事件）单击面板任意一行则手动隐藏**（隐藏态写盘，重启仍隐藏）；再打开一次 `/todos` 即恢复显示。
  面板无键盘快捷键——交互全部走鼠标（0.17.3 起移除原 `ctrl+shift+t`）。首帧锁定高度、实时更新不会缩行（防终端跳动），
  显式展开/收起则允许改高；刷新零轮询（store 变更才触发）。
...
- **交互**：`/todos` = 恢复面板（右键隐藏过的话）+ notify 打印文字任务列表
  （**0.17.5 起移除了原全屏 overlay 弹窗**——面板 + todo 工具已覆盖其用途，不再弹窗挡屏幕）；0.17.6 起新增 **skill-mux**：`/skill:a /skill:b 其他文字` 一条输入展开多个 skill（宿主只认第一个，此处补齐；0.17.7 起 `￥skill名` 是等价快捷触发符，可混用：`￥a /skill:b 文字`；0.17.8 起第二个及以后的 token 也有补全菜单——内置补全器只为首 token 弹菜单，此处补齐，`/` 与 `￥` 均可；0.18.1 起菜单弹出时机：`￥` 按下即弹，第二个及以后的 `/` 也**按下即弹**（根因：宿主只在行首自动触发 `/`，且补全查询返空会清掉菜单状态，状态一死 `/` 按键零查询；0.18.1 由 composer 在插入 `/` 后补发一次查询；0.18.2 去掉了 0.18.0 曾加的状态提示行，该位置不再多显示任何内容）；0.18.3 起多 skill 展开在转写区**合并成一条可折叠的 `[skill] …`**（宿主只解析一个 skill 块，后续块嵌套进第一块内部，模型仍逐字收到全部正文）；**0.19.0 起该条目可点击**——左键单击展开，再次单击折叠（ctrl+o 依旧可用；带 Shift/Ctrl/Alt 的点击仍交给文本选择）；**0.19.2 起折叠行列出全部 skill 名**（`[skill] a + b (ctrl+o to expand)`，展开后的 `**a + b**` 头部同样如此——宿主自身只渲染 `skillBlock.name`，此处从嵌套块里补上其余名字，主题、快捷键提示与正文仍由宿主渲染））；`/todos-doctor`
